...
Direct Gateway `cron.add` callers can supply an `id` when they need an exact creation identity: a second add with that ID fails instead of updating the existing job. After an uncertain response, read that exact job and verify its definition before deciding whether the original add succeeded. For conditional creation, omit `declarationKey`; declaration-keyed adds use convergence semantics and may update an existing job.
