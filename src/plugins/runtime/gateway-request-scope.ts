...
type PluginRuntimeGatewayRequestScope = {
  ...
  /** Exact core methods a current plugin Gateway handler may dispatch in-process. */
...
export function withPluginRuntimePluginScope<T>(scope: PluginRuntimePluginScope, run: () => T): T {
  ...
  if (scope.gatewayMethodDispatchMethods !== undefined) {
    scoped.gatewayMethodDispatchMethods = scope.gatewayMethodDispatchMethods;
  } else {
