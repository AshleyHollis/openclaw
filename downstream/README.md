...
2. `build-downstream-artifact.yml` verifies the exact preserved tarball, freezes the external Codex package, proves CLI registration and a scoped loopback RPC, builds the runtime image, scans it, and optionally publishes it with an SBOM and provenance attestation.
