...
test("ref packaging runs the frozen source's package owner, not the publisher's", {
  ...
  try {
    ...
    for (const relative of ["scripts/resolve-openclaw-package-candidate.mts", "scripts/npm-runner.mjs",
    ...
    const result = spawnSync(process.execPath, ["scripts/resolve-openclaw-package-candidate.mts",
